...
module.exports = async (_deployer, _network, accounts) => {
  // await Promlet balance = await instance.getBalance(accounts[0])
  ...
  const seedTokenBalances = (trader) =>
    [bat, dai, rep, zrx].map((token) => seedTokenBalance(token, trader));

  await Promise.all(
    [trader1, trader2, trader3, trader4].flatMap((trader) =>
      seedTokenBalances(trader)
    )
  );
